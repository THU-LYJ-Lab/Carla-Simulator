...
def solve_single(directory, datafolder, scene_id):
	...
	semseg_directory = os.path.join(directory, 'masks', 'semseg')
	dynamic_mask_directory = os.path.join(directory, 'masks', 'dynamic_object')
	...
	old_dynamic_mask_directory = os.path.join(datafolder, 'insseg')
	...
	mask_folder_name = ['back_right', 'back_left', 'back', 'front_right', 'front_left', 'front']
	for i in tqdm(range(6)):
		for j in tqdm(range(N_FRAMES)):
			frame_id = FRAME_START_NUM + j

			# Save the dynamic
			old_mask = os.path.join(old_dynamic_mask_directory, mask_folder_name[i], 'dynamic_object_mask_frame_%d.npz' % frame_id)
			if not os.path.exists(os.path.join(dynamic_mask_directory, 'camera_' + camera_folder_name_cap[i])):
				os.makedirs(os.path.join(dynamic_mask_directory, 'camera_' + camera_folder_name_cap[i]))
			new_mask = os.path.join(dynamic_mask_directory, 'camera_' + camera_folder_name_cap[i], '%.8d.npz' % j)
			open(new_mask, 'wb').write(open(old_mask, 'rb').read())
