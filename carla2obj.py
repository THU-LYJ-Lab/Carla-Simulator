# Usage: carla2obj input.obj [output.obj]
# This will read from input.obj (carla axis system) then convert it into output.obj (obj axis system)
...
def main():
    ...
    if len(args) > 3 or len(args) < 2:
        print("Usage: carla2obj input.obj [output.obj]")
        print("This will read from input.obj (carla axis system) then convert it into output.obj (obj axis system)")
    ...
    with open(args[2],'w',encoding='utf-8') as w:
        for line in obj_lines:
            if line.startswith('v '):
                ...
                new_point = np.array(line.split()[1:], dtype=np.float64)
                w.write(' '.join(['vn', str(new_point[0]), str(new_point[2]), str(new_point[1])]) + '\n')
